...
Begin VB.Form Question1 
   ...
   Begin VB.Image Image1 
      Height          =   7395
      Left            =   -120
      Picture         =   "Question1.frx":1A5E
      Top             =   -120
      Width           =   12555
   End
End
Attribute VB_Name = "Question1"
...
rs.Open "Select * from Easy2", cn, 1, 2
Do Until rs.EOF
    max = max + 1
    rs.MoveNext
Loop
...
'    Set rs = New ADODB.Recordset
'    With rs
'        .Open "Select * from Easy2 where questionno = " & q_count & "", cn, 1, 2
'        If .EOF = False Then
...
If q_count <= 5 Then
    ...
    Set rs = New ADODB.Recordset
    With rs
        .Open "Select * from Easy2 where questionno = " & strCriteria & "", cn, 1, 2
        If .EOF = False Then
            Me.lblQuestion.Caption = .Fields("question").Value
            Me.lblAns.Caption = .Fields("Answer").Value
        End If
